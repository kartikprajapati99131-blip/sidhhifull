...
/**
 * Returns a human-friendly date group label for a given ISO date string.
 * Groups: Today, Yesterday, This Week, This Month, then "Month Year" buckets.
 */
...
/**
 * A numeric sort key for a date-group label so we can order them newest → oldest.
 */
function dateGroupSortKey(label) {
    ...
    // "June 2025" etc. — parse back to a timestamp (negative so newest < oldest)
    const d = new Date(label);
    return isNaN(d) ? 999 : -d.getTime();
}

/**
 * Groups an array of customers by their `createdAt` date bucket.
 * Returns an array of { label, customers } sorted newest → oldest.
 */
            ...
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6">
                ...
                <div className="flex flex-col gap-4">
                    <Field label="Full Name *">
                        <input type="text" placeholder="e.g. Rahul Shah" value={form.name} onChange={set("name")}
                    ...
                    <Field label="Address">
                        <textarea placeholder="e.g. 12, Rander Road, Surat 395009" value={form.address} onChange={set("address")}
                            rows={2} className="input-base resize-none" />
                    </Field>
                    <Field label="Notes / Remarks">
                        <textarea placeholder="Any extra info..." value={form.notes} onChange={set("notes")}
                            rows={2} className="input-base resize-none" />
                    </Field>
                    ...
                    <div className="flex gap-3 mt-1">
                        ...
                        <button onClick={handleSave} disabled={saving}
                            className="flex-1 bg-gray-900 rounded-xl py-2.5 text-sm font-semibold text-white hover:bg-gray-700 transition disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <Field label="Address">
                        <textarea value={form.address} onChange={set("address")} rows={2} className="input-base resize-none" />
                    </Field>
                    <Field label="Notes / Remarks">
            ...
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6">
                            ...
                            value={
                                <span className="flex items-center gap-2">
                                    ...
                                    {customer.addedByRole && (
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full capitalize ${ROLE_STYLES[customer.addedByRole] || "bg-gray-100 text-gray-600"}`}>
                                            {customer.addedByRole}
                                        </span>
                                    )}
                                </span>
                ...
                {isAdmin && (
                    <div className="flex gap-3">
                        <button
                            onClick={() => { onClose(); onEditOpen(customer); }}
                        ...
                        <button
                            ...
                            disabled={deleting}
                            className="flex-1 border border-rose-200 text-rose-500 rounded-xl py-2.5 text-sm font-semibold hover:bg-rose-50 transition disabled:opacity-50"
                        >
...
function CustomerCard({ customer, isAdmin, onClick }) {
    return (
        <button
            onClick={onClick}
            className="w-full text-left bg-white border border-gray-100 rounded-2xl px-5 py-4 shadow-sm hover:shadow-md hover:border-gray-200 transition flex items-center gap-4"
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isAdmin && customer.addedByRole && (
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full capitalize flex-shrink-0 ${ROLE_STYLES[customer.addedByRole] || "bg-gray-100 text-gray-600"}`}>
                            {customer.addedByRole}
                        </span>
                    )}
                </div>
                ...
                {isAdmin && customer.addedByName && (
                    <p className="text-xs text-gray-300 mt-0.5 truncate">by {customer.addedByName}</p>
                )}
            </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm px-5 py-16 text-center">
            ...
            {!search && (
                <>
                    ...
                    <button
                        onClick={onAdd}
                        className="inline-flex items-center gap-2 bg-gray-900 text-white text-sm font-semibold px-5 py-2.5 rounded-xl hover:bg-gray-700 transition"
                    >
...
export default function CustomersPage() {
    ...
    const filtered = customers.filter((c) => {
        ...
        return (
            ...
            c.addedByName?.toLowerCase().includes(q)
        );
    });
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            {selectedCustomer && !editingCustomer && (
                <CustomerDetailModal
                    ...
                    onDelete={(id) => setCustomers((prev) => prev.filter((c) => c._id !== id))}
                    onEditOpen={(c) => { setSelected(null); setEditing(c); }}
                />
            )}
            ...
            <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="flex items-start justify-between mb-6 gap-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-0.5">
                            {isAdmin
                                ...
                                : `Your customers · ${customers.length} added`}
                        </p>
                        {!isAdmin && userRole && (
                            <span className={`mt-2 inline-block text-xs font-bold px-3 py-1 rounded-full capitalize ${ROLE_STYLES[userRole] || "bg-gray-100 text-gray-600"}`}>
                                {userRole}
                            </span>
                        )}
                    </div>
                    <button
                        ...
                        className="flex items-center gap-2 bg-gray-900 text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-gray-700 transition flex-shrink-0"
                    >
                ...
                {isAdmin && (
                    ...
                    <div className="relative mb-5">
                        ...
                        <input
                            type="text"
                            placeholder={isAdmin ? "Search by name, phone, address or staff..." : "Search by name, phone or address..."}
                            value={search}
