...
export async function computeSalarySummary({ from, to, userId }) {
  ...
  for (const user of staffUsers) {
    ...
    // ── Compensation Hours — fetched before pay math, because they
    // feed directly into baseIncome/totalIncome via totalEffectiveHours. ──
    const { compensationHours } = await getCompensationSummary({ employeeId: uid, from, to });
    ...
    const baseIncome = hourlyRate * totalEffectiveHours;
